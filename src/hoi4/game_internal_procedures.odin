...
jsonGetColor_field :: proc(root: json.Object, path: string, $field: string) -> (res: Color, is_successful: bool) {
	color_array, is_an_array := jsonGetField(root, path, field, json.Array)
	if (!is_an_array) { is_successful = false; return }
...
jsonGetColor_color :: #force_inline proc(root: json.Object, path: string) -> (res: Color, is_successful: bool) {
	return jsonGetColor(root, path, "color")
}

jsonGetColor_vec4 :: proc {
	jsonGetColor_vec4_field,
	jsonGetColor_vec4_color
}

jsonGetColor_vec4_field :: #force_inline proc(root: json.Object, path: string, $field: string) -> (res: [4]f32, ok: bool) {
	c, is_successful := jsonGetColor(root, path, field)
...
jsonGetColor_vec4_color :: #force_inline proc(root: json.Object, path: string) -> (res: [4]f32, ok: bool) {
	return jsonGetColor_vec4(root, path, "color")
}
